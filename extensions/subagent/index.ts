...
function readSubagentSettings(): SubagentSettings {
  try {
    ...
    if (raw?.subagent && typeof raw.subagent === "object")
      return raw.subagent as SubagentSettings;
  } catch {
...
const SubagentParams = Type.Object({
  ...
  stages: Type.Optional(
    Type.Array(PipelineStage, {
      description:
        "Pipeline mode: ordered stages each item passes through. Use with `items`.",
    }),
  ),
...
export default function (pi: ExtensionAPI) {
  pi.registerTool({
    ...
    async execute(_toolCallId, params, signal, onUpdate, ctx) {
      ...
      const hasPipeline =
        (params.items?.length ?? 0) > 0 && (params.stages?.length ?? 0) > 0;
      const modeCount =
        Number(hasChain) +
        Number(hasTasks) +
        Number(hasSingle) +
      ...
      if (
        ...
        if (params.stages)
          for (const s of params.stages) requestedAgentNames.add(s.agent);
      ...
      if (hasPipeline && params.items && params.stages) {
        ...
        const finals = await mapWithConcurrencyLimit(
          items,
          MAX_CONCURRENCY,
          async (item, idx) => {
            let previous = "";
        ...
        const successCount = finals.filter(
          (r) => r && !isFailedResult(r),
        ).length;
        const summaries = finals.map((r, i) => {
          ...
          const status = isFailedResult(r)
            ...
            : "completed";
          const label =
            items[i].length > 50 ? `${items[i].slice(0, 50)}…` : items[i];
          return `### [${label}] ${status}\n\n${output}`;
        });
        return {
          content: [
    ...
    renderCall(args, theme, _context) {
      ...
      if (
        ...
        for (const s of args.stages.slice(0, 3)) {
          ...
          const preview =
            clean.length > 36 ? `${clean.slice(0, 36)}...` : clean;
    ...
    renderResult(result, { expanded }, theme, _context) {
      ...
      if (details.mode === "parallel" || details.mode === "pipeline") {
        const modeLabel =
          details.mode === "pipeline" ? "pipeline " : "parallel ";
  ...
  pi.registerCommand("subagent", {
    description:
      "Show subagent model routing (tiers → models) and how each agent resolves",
