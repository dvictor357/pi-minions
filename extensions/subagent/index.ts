...
/**
 * Read the `subagent` block from settings.json. Shape:
 *   "subagent": {
 ...
 */
function resolveAgentRuntime(agent: AgentConfig): { model?: string; thinking?: string } {
...
async function runAgentWithRetry(
	...
	let result = await runSingleAgent(defaultCwd, agents, agentName, task, cwd, step, signal, onUpdate, makeDetails);
	let attempt = 0;
	while (
		...
		attempt++;
		result = await runSingleAgent(defaultCwd, agents, agentName, task, cwd, step, signal, onUpdate, makeDetails);
	}
	return result;
}
...
const PipelineStage = Type.Object({
	...
	task: Type.String({
		description: "Stage instruction. {item} = the current item; {previous} = prior stage's output for this item.",
	}),
	cwd: Type.Optional(Type.String({ description: "Working directory for the agent process" })),
});
...
const SubagentParams = Type.Object({
	...
	items: Type.Optional(
		Type.Array(Type.String(), {
			description: "Pipeline mode: list of items. Each item flows through every `stages` step independently (no barrier between stages — item B can be in stage 1 while item A is in stage 3). Requires `stages`.",
		}),
	),
...
export default function (pi: ExtensionAPI) {
	pi.registerTool({
		...
		async execute(_toolCallId, params, signal, onUpdate, ctx) {
			...
			if (params.chain && params.chain.length > 0) {
				...
				for (let i = 0; i < params.chain.length; i++) {
					...
					previousOutput = isError ? getResultOutput(result) : getFinalOutput(result.messages);
				}
				return {
			...
			if (hasPipeline && params.items && params.stages) {
				...
				if (items.length > MAX_PIPELINE_ITEMS)
					return {
						content: [
							{ type: "text", text: `Too many pipeline items (${items.length}). Max is ${MAX_PIPELINE_ITEMS}.` },
						],
				...
				const emitPipelineUpdate = () => {
					...
					onUpdate({
						content: [{ type: "text", text: `Pipeline: ${done}/${items.length} done, ${running} running...` }],
				...
				const finals = await mapWithConcurrencyLimit(items, MAX_CONCURRENCY, async (item, idx) => {
					...
					for (let s = 0; s < stages.length; s++) {
						...
						const task = stage.task.replace(/\{item\}/g, item).replace(/\{previous\}/g, previous);
						...
						previous = getFinalOutput(r.messages);
					}
		...
		renderCall(args, theme, _context) {
			...
			if (args.items && args.items.length > 0 && args.stages && args.stages.length > 0) {
				let text =
					theme.fg("toolTitle", theme.bold("subagent ")) +
					theme.fg("accent", `pipeline (${args.items.length} items × ${args.stages.length} stages)`) +
					theme.fg("muted", ` [${scope}]`);
	...
	pi.registerCommand("subagent", {
		description: "Show subagent model routing (tiers → models) and how each agent resolves",
		handler: async (_args, ctx) => {
			...
			if (tiers.size === 0) {
				lines.push("  (none configured — agents use their explicit model/thinking, or pi defaults)");
			} else {
				for (const t of tiers) {
					lines.push(`  ${t}: ${models[t] ?? "(pi default model)"}${thinking[t] ? ` · think:${thinking[t]}` : ""}`);
				}
			}
			...
			for (const a of [...agents].sort((x, y) => x.name.localeCompare(y.name))) {
				...
				const via = a.tier ? `[${a.tier}]` : a.model ? "[explicit]" : "[default]";
				lines.push(`  ${a.name} ${via} → ${r.model ?? "(pi default)"}${r.thinking ? ` · think:${r.thinking}` : ""}`);
			}
